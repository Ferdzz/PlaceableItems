...
public class BlockPlaceableItems extends Block implements IBlockPlaceableItems {
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn) {
		...
		if(!canBlockStay(worldIn, pos, state)) {
			...
			if(state.getProperties().get(BlockEdible.PLATED) != null && state.getValue(BlockEdible.PLATED)) {
				EntityItem drop = new EntityItem(worldIn, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, new ItemStack(ModItems.itemPlate));
	...
	public void onNeighborChange(IBlockAccess world, BlockPos pos, BlockPos neighbor) {
		IBlockState state = world.getBlockState(pos); // TODO: Make sure this event is called properly when destroying the bottom block
		neighborChanged(state, (World) world, pos, state.getBlock());
	}
	
	@Override
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		ItemStack is = playerIn.getHeldItem(hand);
		return onBlockActivated(worldIn, pos, state, playerIn, hand, is, facing, hitX, hitY, hitZ);
	}
